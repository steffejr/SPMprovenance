...
BaseDir = fullfile(BaseDir,'SPMprovenance/OutputFiles');

InFile = 'iLS_PreProcss_NoFieldMap_FilledInJobMatLabLibrary.provn';
        ...
        if findStr(1) == 1
            ...
            elseif length(temp)>(length(StructName)+1)
                ...
                if ~isempty(findProvValue)
                    ...
                    % if there is no value then assume that this is a
                    % substructure
                    Str = sprintf('%s{%d}%s=%s;',StructName,step,subStructPath,'{}');
                    eval(Str)
                end
            end
        else
